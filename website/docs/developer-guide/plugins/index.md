...
| `pre_kanban_complete` | Before the shared Kanban completion transition, for tools, CLI, and direct API callers | `task_id: str, board: str \| None, assignee: str \| None, summary: str` | `None` declines; `{"action": "block", "message": "..."}` vetoes. Any block wins over allow/approve results. |
