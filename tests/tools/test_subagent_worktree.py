...
class SubagentWorktreeTests(unittest.TestCase):
    ...
    def test_create_uses_parent_head_not_remote_default(self):
        repo = _make_repo(self.tmp)
        ...
        feature_sha = _git(["rev-parse", "HEAD"], repo).stdout.strip()

        info = sw.create_subagent_worktree(str(repo), "parent-head")

        self.assertIsNotNone(info)
        assert info is not None
        self.assertEqual(info["base_commit"], feature_sha)
        self.assertEqual(_git(["rev-parse", "HEAD"], Path(info["path"])).stdout.strip(), feature_sha)
        self.assertTrue((Path(info["path"]) / "feature.txt").exists())
