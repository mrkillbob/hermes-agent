...
def _sub(conn, task_id, *, chat_id="trading-bot", thread_id=""):
    kb.add_notify_sub(
        conn,
        task_id=task_id,
        platform="discord",
...
def _task(conn, title, *, parents=(), status="ready", branch_name=None):
...
def _source(*, chat_id="trading-bot", thread_id=None):
    from gateway.progress_queries import ProgressSource

    return ProgressSource(platform="discord", chat_id=chat_id, thread_id=thread_id)
