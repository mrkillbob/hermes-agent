    ...
    *,
    ...
    for row in rows:
        ...
        try:
            ...
            continue
        finding = detect_log_finding(text or "", config)
        if finding is None:
            continue
        attempt_count = int(
            conn.execute(
        ...
        # A repair may need to inspect the original workspace, but it must not
        # own that workspace's lifecycle. ``dir`` is the non-cleaning borrowed
        # workspace kind; this prevents repair completion from deleting an
        # original scratch directory or removing an original git worktree.
        repair_workspace_kind = "dir" if task.workspace_path else "scratch"
        repair_workspace_path = task.workspace_path if task.workspace_path else None
        try:
