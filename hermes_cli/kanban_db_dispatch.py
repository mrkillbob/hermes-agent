...
# A single terminal command must not consume an entire Kanban attempt.  The
# terminal tool's foreground safety cap is 600s; keeping the worker default at
# or below that cap prevents an ordinary foreground call from being promoted
# to an unbounded background process that can keep renewing the Kanban lease.
KANBAN_WORKER_COMMAND_TIMEOUT_SECONDS = 600
...
def _worker_terminal_timeout_env(
    ...
    Keep each individual terminal command bounded below the task runtime.  A
    previous implementation raised this default to ``max_runtime - grace``;
    that let one hung command occupy a worker for the whole attempt, while
    tool activity kept the lease heartbeat fresh.  Preserve a shorter existing
    timeout, but lower a longer inherited/profile timeout to the Kanban command
    cap so the worker must return to the agent loop and make a terminal
    decision.
    """
    ...
    desired = min(
        max(1, runtime - KANBAN_TERMINAL_TIMEOUT_GRACE_SECONDS),
        KANBAN_WORKER_COMMAND_TIMEOUT_SECONDS,
    )
    try:
        existing = int(str(current_timeout).strip()) if current_timeout else 0
    except (TypeError, ValueError):
        existing = 0
    if 0 < existing <= desired:
        return None
    return str(desired)
