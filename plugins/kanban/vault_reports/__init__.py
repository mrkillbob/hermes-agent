...
Standalone plugin: install as a standalone plugin repo under ~/.hermes/plugins/
or as a pip entry point. Repository policy prohibits third-party connectors in
the core tree; this module ships here only as a plugin behind the plugin boundary.
"""
...
def append_vault_context(
    ...
    if not snippets:
        return response
    snippet_text = "\n".join(snippets)[:max(0, 2200 - len(response) - 2)]
    return response + "\n\n" + snippet_text
