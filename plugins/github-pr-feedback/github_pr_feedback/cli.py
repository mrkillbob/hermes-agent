...
from .stack import StackEntry, StackStore
...
def setup_cli(_ctx: Any, parser: argparse.ArgumentParser) -> None:
    ...
    scan.add_argument(
        "--repository",
        help="Limit the bounded scan/coordinator pass to one configured repository",
    )
    audit_backlog = subcommands.add_parser(
        "audit-backlog",
        help="Dispatch the next bounded exact-head local-CI audit for one repository",
    )
    audit_backlog.add_argument("--repository", required=True)
    ci_repair_backlog = subcommands.add_parser(
        "ci-repair-backlog",
        help="Dispatch bounded repair work from typed failed exact-head CI receipts",
    )
    ci_repair_backlog.add_argument("--repository", required=True)
    ci_repair_backlog.add_argument("--limit", type=int, default=1)
    status = subcommands.add_parser("status", help="Show durable receipt counts")
    status.add_argument(
        "--repository",
        help="Accepted for repository-scoped coordinator scripts; counts remain aggregate",
    )
    ...
    merge_scan = subcommands.add_parser(
        "merge-scan", help="Evaluate eligible PR heads repo-wide or for one PR/stack"
    )
    merge_scan.add_argument("--repository")
    merge_scan.add_argument("--pr-number", type=int)
    merge_scan.add_argument("--stack-id")
    merge_pr = subcommands.add_parser("merge-pr", help="Evaluate and merge one explicitly selected PR")
    merge_pr.add_argument("--repository", required=True)
    merge_pr.add_argument("--pr-number", required=True, type=int)
    merge_pr.add_argument("--stack-id")
    ...
    deployment_retry = subcommands.add_parser(
        "deployment-retry",
        help="Retry one failed post-merge deployment from its immutable merge receipt",
    )
    deployment_retry.add_argument("--repository", required=True)
    deployment_retry.add_argument("--pr-number", required=True, type=int)
    ...
    native_stack_refresh = subcommands.add_parser(
        "stack-native-refresh",
        help="Headlessly rebase and push a GitHub-native stack selected by PR",
    )
    native_stack_refresh.add_argument("--repository", required=True)
    native_stack_refresh.add_argument("--pr-number", required=True, type=int)
    native_stack_refresh.add_argument("--repository-path", required=True, type=Path)
...
def handle_cli_with_context(ctx: Any, args: argparse.Namespace) -> int:
    ...
    if action == "audit-backlog":
        return _scan(ctx, args, audit_backlog=True)
    if action == "ci-repair-backlog":
        return _ci_repair_backlog(ctx, args)
    if action == "status":
        return _status(repository=getattr(args, "repository", None))
    if action == "doctor":
        return _doctor(ctx)
    ...
    if action == "merge-scan":
        return _merge_scan(ctx, args)
    if action == "merge-pr":
        return _merge_scan(ctx, args)
    if action == "merge-status":
        return _merge_status(details=bool(getattr(args, "details", False)))
    if action == "deployment-retry":
        return _deployment_retry(ctx, args)
    ...
    if action == "stack-native-refresh":
        return _stack_native_refresh(ctx, args)
...
def _scan(
    ...
    *,
    ...
    if not policy.enabled:
        ...
        return 0
    repository_filter = (
        str(getattr(args, "repository", "") or "").strip()
        or os.environ.get("HERMES_PR_FEEDBACK_REPOSITORY", "").strip()
    )
        ...
        try:
            ...
            if repository_filter:
                result = controller.scan(
                    apply_labels=False, repository_filter=repository_filter
                )
            else:
            ...
            if policy.repair_steward is not None:
                ...
                if repository_filter:
                    repair = repair_controller.scan(
                        conflicts_only=required_ci_backlog,
            ...
            if policy.merge_policies() and not audit_backlog:
                merge_payload = _run_merge_scan(
                    policy, ledger, repository=repository_filter or None
                )
            release_policies = (
            ...
            if release_policies and not required_ci_backlog and not audit_backlog:
                maintenance_results = [
...
def _status(*, repository: str | None = None) -> int:
    ledger = FeedbackLedger.for_current_profile()
    try:
        counts = ledger.status_counts(repository=repository)
    finally:
        ledger.close()
...
def _ci_repair_backlog(ctx: Any, args: argparse.Namespace) -> int:
    """Dispatch current-head repair work from typed failed CI receipts only."""

    try:
        policy = _load_policy_from_context(ctx)
        target = policy.targets.get(args.repository)
        if not policy.enabled or target is None:
            raise ValueError("repository is not a configured target")
        if not isinstance(args.limit, int) or isinstance(args.limit, bool) or not 1 <= args.limit <= 10:
            raise ValueError("limit must be between 1 and 10")
        ledger = FeedbackLedger.for_current_profile()
        try:
            receipts = ledger.failed_ci_receipts(
                args.repository, limit=min(10, args.limit * 10)
            )
            controller = _controller(policy, ledger)
            outcomes = []
            for receipt in receipts:
                outcome = {
                    ...
                    "status": controller.dispatch_ci_failure(receipt),
                }
                outcomes.append(outcome)
                if outcome["status"] in {"scheduled", "duplicate"}:
                    if sum(item["status"] in {"scheduled", "duplicate"} for item in outcomes) >= args.limit:
                        break
        finally:
            ledger.close()
    except (GitHubClientError, LedgerStateError, TypeError, ValueError) as error:
        print(json.dumps({"status": "ci_repair_backlog_unavailable", "reason": str(error)}, sort_keys=True))
        return 1
    print(json.dumps({"status": "ok", "repository": args.repository, "dispatched": outcomes}, sort_keys=True))
    return 0
    ...
    try:
        ...
        try:
            ...
            if receipt.status == "failed":
                # ``audit_only`` suppresses comments and merge handoff; it must
                # not suppress the typed repair handoff. Otherwise every
                # failed local audit becomes a permanent merge blocker with no
                # path to a configured CI fixer.
...
def _merge_scan(ctx: Any, args: argparse.Namespace | None = None) -> int:
    args = args or argparse.Namespace()
    repository = getattr(args, "repository", None)
    pr_number = getattr(args, "pr_number", None)
    if pr_number is not None and (not repository or pr_number < 1):
        print(json.dumps({"status": "invalid_selector"}, sort_keys=True))
        return 1
    ...
    if stack_id:
        if not repository:
            ...
            return 1
        try:
            manifest = StackStore(get_default_hermes_root() / "github-pr-feedback" / "stacks").load(repository, stack_id)
            selected_numbers = tuple(
                entry.pr_number for entry in _ordered(manifest.entries, manifest.base_branch)
                if entry.pr_number is not None
            )
            if not selected_numbers:
                raise ValueError("stack has no pull requests")
        except (TypeError, ValueError):
            print(json.dumps({"status": "invalid_stack_selector"}, sort_keys=True))
            return 1
    elif pr_number is not None:
        selected_numbers = (pr_number,)
    try:
        policy = _load_policy_from_context(ctx)
    except ValueError:
        print(json.dumps({"status": "invalid_configuration"}, sort_keys=True))
        return 1
    if repository and repository not in {item.repository for item in policy.merge_policies()}:
        print(json.dumps({"status": "repository_not_configured", "repository": repository}, sort_keys=True))
        return 1
    if not policy.merge_policies():
        print(json.dumps({"status": "disabled"}, sort_keys=True))
        return 0
    ledger = FeedbackLedger.for_current_profile()
    try:
        payload = _run_merge_scan(policy, ledger, repository=repository, selected_numbers=selected_numbers)
    finally:
        ledger.close()
    print(json.dumps(payload, sort_keys=True))
...
def _run_merge_scan(
    ...
    *,
    ...
    if repository:
        merge_policies = tuple(item for item in merge_policies if item.repository == repository)
    ...
    if len(merge_policies) == 1:
        return _run_merge_scan_for_policy(
            policy, merge_policies[0], ledger, selected_numbers=selected_numbers,
            github=github, kanban=kanban
        )
    results = [
...
def _run_merge_scan_for_policy(
    ...
    *,
    ...
    if selected_numbers is not None:
        requested = set(selected_numbers)
        pull_requests = tuple(pr for pr in pull_requests if pr.number in requested)
...
def _announce_ready_to_merge(github: GitHubClient, repository: str, pull_request) -> None:
    ...
    try:
        ...
        if (
            readback.head_sha.casefold() != pull_request.head_sha.casefold()
...
def _merge_maintainer_task(
    policy: PluginPolicy, merge_policy, pull_request, decision: MergeDecision
) -> KanbanTask:
...
def _deployment_retry(ctx: Any, args: argparse.Namespace) -> int:
    """Retry only a recorded failed deployment for a completed governed merge."""

    try:
        policy = _load_policy_from_context(ctx)
        merge_policy = policy.merge_policy_for(args.repository)
        if merge_policy is None or merge_policy.post_merge is None:
            raise ValueError("post-merge deployment is not configured for this repository")
        if args.pr_number < 1:
            raise ValueError("pr_number must be positive")
        ledger = FeedbackLedger.for_current_profile()
        try:
            merge = ledger.completed_merge_receipt(args.repository, args.pr_number)
            deployment = ledger.latest_deployment_receipt(args.repository, args.pr_number)
            if not isinstance(merge, MergeReceipt):
                raise ValueError("completed merge receipt is unavailable")
            if getattr(deployment, "status", None) != "failed":
                raise ValueError("failed deployment receipt is unavailable")
            if deployment.merge_commit_oid != merge.merge_commit_oid:
                raise LedgerStateError("deployment receipt does not match the completed merge")
            result = PostMergeExecutor(merge_policy.post_merge, ledger).run(merge)
        finally:
            ledger.close()
        ...
        print(
            json.dumps(
                {
                    "status": "deployment_retry_unavailable",
                    "repository": args.repository,
                    "pr_number": args.pr_number,
                    "reason": str(error),
                },
...
def _stack_native_refresh(ctx: Any, args: argparse.Namespace) -> int:
    try:
        policy = _load_policy_from_context(ctx)
        pull = StackController(policy, github=_github_client(policy)).refresh_native(
            args.repository,
            args.pr_number,
            repository_path=args.repository_path,
        )
