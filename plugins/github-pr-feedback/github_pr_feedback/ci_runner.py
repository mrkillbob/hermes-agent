...
class LocalCIRunner:
    ...
    def _run_claimed(self, identity: CIAuditIdentity, worktree: Path) -> CIAuditReceipt:
        ...
        python_argv = (".venv/bin/python",) if python_path.is_file() and os.access(python_path, os.X_OK) else self._python_argv
        if not python_path.is_file() and self._python_argv == (".venv/bin/python",):
            raise CIValidationError("exact audit worktree Python environment is missing")

        started_at = _aware_now(self._now())
        command_specs: list[tuple[tuple[str, ...], Path, dict[str, str]]] = [
            (python_argv + ("scripts/check_ci_governance.py",), worktree, {}),
            (
                python_argv + ("scripts/run_static_lane.py",),
                worktree,
                {"STATIC_BASE_REF": identity.base_sha},
            ),
            (python_argv + ("scripts/run_hygiene_lane.py",), worktree, {}),
        ]
        command_specs.extend(
            (
                _lane_argv(python_argv, lane, identity),
                worktree,
                {},
            )
            for lane in lanes
