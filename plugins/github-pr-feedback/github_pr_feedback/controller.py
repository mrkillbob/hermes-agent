...
def _task(
    ...
    *,
    ...
    instructions = (
        ...
        "expected receipt SHA; otherwise stop fail-closed. If the governed push-head command returns "
        "status=reconciliation_pending after the push, treat that as a recoverable confirmation "
        "failure, not as supersession: retain its returned head_sha, do not retry push-head, re-read "
        "the canonical PR, and continue only if its head equals that returned pushed head. Use that "
        "resolved head for the factual reply and complete-feedback. For every later GitHub write, "
        "require the canonical PR head to equal the resolved head; stop fail-closed on any other value. "
