...
def _mark_pooled_worktree_dispatching(
    local_git: object, receipt: FeedbackReceipt, idempotency_key: str
) -> None:
    """Best-effort: if local_git is a worktree pool, record that a Kanban

    task-creation call is about to be made for this receipt's leased slot,
    *before* actually calling create_or_get_task(). Must run before every
    create_or_get_task() call at a pooled dispatch site, mirroring
    _bind_pooled_worktree_task's placement right after it: together they
    bracket the idempotent create_or_get_task() call with ledger evidence,
    so a dispatcher dying between the two still leaves proof that task
    creation was attempted (and may have already succeeded) instead of
    looking identical to a lease nobody ever tried to dispatch.  A plain
    (non-pooled) LocalGitRepository has no such method, so this is a silent
    no-op for it.
    """

    mark_dispatching = getattr(local_git, "mark_task_dispatching", None)
    if callable(mark_dispatching):
        mark_dispatching(receipt, idempotency_key)
...
class LocalGitRepository:
    ...
    def prepare_maintenance_worktree(
        ...
        if created:
            try:
                from hermes_cli.worktree_environment import (
                    bootstrap_worktree_environments,
                )
            except ImportError:
                # hermes_cli.worktree_environment is not yet present on every
                # branch this plugin runs from (e.g. before it lands via a
                # sibling in-flight change). Venv bootstrap is an
                # optimization for a freshly created maintenance worktree,
                # not a correctness requirement -- skip it rather than fail
                # the whole maintenance dispatch.
                pass
            else:
                bootstrap_worktree_environments(
...
def _pid_is_alive(pid: int) -> bool:
    """Read-only liveness check for a ledger-recorded dispatcher process."""
...
class PooledLocalGitRepository:
    ...
    def prepare_receipt_worktree(
        ...
        if not _SHA.fullmatch(receipt.head_sha):
            raise ValueError("head SHA is not a full Git object ID")
        self._ensure_exact_head(path, receipt)
        self._ledger.reclaim_dead_unbound_worktree_slots(self._pid_is_alive)
    ...
    def mark_task_dispatching(
        self, receipt: FeedbackReceipt, idempotency_key: str
    ) -> None:
        """Record that a Kanban task-creation call is about to be made for

        this receipt's leased slot, called right before create_or_get_task()
        at every pooled dispatch site. See
        FeedbackLedger.mark_worktree_slot_dispatching for why this has to
        happen *before* the call rather than only recording the outcome
        after: it closes the crash window between an idempotent
        create_or_get_task() success and bind_task() recording it.
        """

        self._ledger.mark_worktree_slot_dispatching(receipt.head_sha, idempotency_key)
...
class ScanController:
    ...
    def dispatch_ci_failure(self, audit: object) -> str:
        ...
        try:
            ...
            _mark_pooled_worktree_dispatching(
                self._local_git, receipt, task.idempotency_key
            )
