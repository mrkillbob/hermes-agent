...
def _hermes_review_task(
    ...
    return KanbanTask(
        ...
        instructions=(
            ...
            "a tool error or with narrative text only. Use the assigned Kanban workspace for all "
            "inspection and tests; never create an additional temporary Git worktree because it "
            "does not inherit the assigned workspace's verified Python environment. If the "
            "required toolchain is absent, record that check as environment-blocked and decide "
            "from the remaining exact-head evidence. If GitHub rejects the normal approval or "
