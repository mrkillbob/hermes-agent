...
class FeedbackLedger:
    ...
    def claim_worktree_slot(
        ...
        *,
        ...
        Re-leasing a previously-bound slot clears its prior occupant's
        task_id/board/dispatch_key: those columns describe *this* lease's
        dispatch, not the one before it, and a stale task_id would hide a
        freshly (re-)leased, still-unbound slot from
        reclaim_dead_unbound_worktree_slots's `task_id IS NULL` filter --
        stranding it until the full lease timeout instead of being
        recognized as abandoned if its new dispatcher also dies unbound.
        """
        ...
        with self._transaction():
            ...
            if row is not None:
                ...
                self._connection.execute(
                    ...
                    "task_id = NULL, board = NULL, dispatch_key = NULL "
                    "WHERE slot_id = ?",
    ...
    def reclaim_dead_unbound_worktree_slots(
        self, pid_is_alive: Callable[[int], bool | None]
    ) -> int:
        """Release slots abandoned before they were bound to a Kanban task.

        A lease with a task binding may legitimately outlive the dispatcher
        that acquired it, so it is governed by task reconciliation and the
        lease timeout.  An unbound lease whose owner process is conclusively
        gone cannot complete that binding and is safe to reclaim immediately
        -- *unless* a task-creation attempt for it may already have gone
        through: create_or_get_task() is idempotent and can succeed just
        before the dispatcher dies, orphaning a worker that is actively
        using the workspace without ever recording task_id on the lease.
        mark_worktree_slot_dispatching() records a dispatch_key the instant
        that attempt begins, *before* create_or_get_task() is called, so a
        lease carrying one cannot be conclusively ruled unbound-and-abandoned
        from local state alone: task creation may have already succeeded.
        Such a lease is left to the full lease timeout instead, the same as
        a lease that did complete its binding. Only a lease with neither a
        task_id nor a dispatch_key -- meaning no dispatch was ever attempted
        for it -- is safe to reclaim the instant its owner PID is gone.
        Unknown liveness remains fail-closed.
        """
        ...
        with self._transaction():
            rows = self._connection.execute(
                ...
                "WHERE status = 'leased' AND task_id IS NULL AND dispatch_key IS NULL"
            ).fetchall()
            for slot_id, version, owner_pid in rows:
                if pid_is_alive(int(owner_pid)) is not False:
                    continue
                result = self._connection.execute(
                    "UPDATE worktree_pool_slots SET status = 'free', updated_at = ? "
                    "WHERE slot_id = ? AND lease_version = ? AND owner_pid = ? "
                    "AND status = 'leased' AND task_id IS NULL AND dispatch_key IS NULL",
                    (
                        _aware_utc(datetime.now(UTC), "updated_at").isoformat(),
                        int(slot_id),
                        int(version),
                ...
                released += int(result.rowcount == 1)
        return released

    def mark_worktree_slot_dispatching(self, head_sha: str, dispatch_key: str) -> None:
        """Record that a Kanban task-creation call is about to be made for

        this leased slot's receipt, *before* actually calling
        create_or_get_task(). This closes the window where the dispatcher
        dies right after create_or_get_task() idempotently succeeds but
        before bind_worktree_slot_task() records task_id: with a
        dispatch_key on file, reclaim_dead_unbound_worktree_slots can no
        longer treat the still-unbound lease as conclusively abandoned just
        because its owner PID is dead, since task creation may have already
        gone through and a worker may already be using the workspace.

        Best-effort by design, matching bind_worktree_slot_task: if the slot
        was already reconciled away (or a non-pooled LocalGit is in use and
        no such slot exists), this is a silent no-op.
        """

        dispatch_key = dispatch_key.strip() if isinstance(dispatch_key, str) else ""
        if not dispatch_key:
            raise ValueError("dispatch_key must be a non-empty string")
        with self._transaction():
            self._connection.execute(
                "UPDATE worktree_pool_slots SET dispatch_key = ? "
                "WHERE head_sha = ? AND status = 'leased'",
                (dispatch_key, head_sha),
            )
