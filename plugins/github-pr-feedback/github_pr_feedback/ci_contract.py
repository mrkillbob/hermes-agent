...
def manifest_path(worktree: Path) -> Path:
    ...
    if legacy.is_file():
        return legacy
    try:
        project = tomllib.loads((worktree / 'pyproject.toml').read_text(encoding="utf-8-sig"))['project']
...
def hermes_commands(worktree: Path, base_sha: str, head_sha: str, changed: tuple[str, ...]):
    ...
    locked_packages = json.loads(root_lock.read_text(encoding="utf-8-sig")).get('packages', {}) if root_lock.is_file() else {}
    ...
    for package in ('apps/desktop', 'apps/shared', 'ui-tui', 'web', 'website'):
        ...
        scripts = json.loads((root / 'package.json').read_text(encoding="utf-8-sig")).get('scripts', {})
