...
def register_repair_completion_policy(ctx):
    ...
    if callable(register_hook):
        try:
            ...
            # Older Hermes runtimes do not expose the completion-policy hook.
            # Keep the plugin loadable so deterministic CI audits can still run;
            # newer runtimes register the full completion guard above.
            return None
    return None
