...
def test_readiness_requires_every_current_head_gate() -> None:
    evidence = ReadinessEvidence(
        ...
        codex_reviewed=True,
        codex_clean=True,
        no_pending_repairs=True,
        no_pending_intent=True,
    )

    assert evaluate_readiness(evidence).eligible is True
    assert evaluate_readiness(evidence.with_changes(feedback_clear=False)).blockers == (
        "feedback_not_clear",
    )
...
def test_codex_clean_heads_are_first_then_oldest_ready() -> None:
    queue = order_ready_queue(
        [_ready(3, ready_at=30), _ready(2, codex_clean=True, ready_at=99), _ready(1, ready_at=10)]
    )
