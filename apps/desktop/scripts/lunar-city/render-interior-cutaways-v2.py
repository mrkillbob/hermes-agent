...
scene=bpy.data.scenes['Lunar City Interior Studies v2'];bpy.context.window.scene=scene
...
scene.render.filepath=str(out/'blender-gallery-v2.png');bpy.ops.render.render(write_still=True)
...
cam.location=center+Vector((14,-18,20));cam.rotation_euler=(center+Vector((0,0,1))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=24;scene.render.resolution_x=1200;scene.render.resolution_y=1000;scene.render.filepath=str(out/'beaver-cutaway-v2.png');bpy.ops.render.render(write_still=True)
