...
def process(asset):
 ...
 before=set(bpy.data.objects);bpy.ops.import_scene.gltf(filepath=str(folder/'source-color-study-v2.glb'))
...
remaining=iter([a for a in ASSETS if not (ROOT/a/'material-review.json').exists()])
def tick():
 try:asset=next(remaining)
 except StopIteration:return None
 try:
  (ROOT/'material-batch-status.json').write_text(json.dumps({'asset':asset,'stage':'processing'})+'\n')
  process(asset)
  (ROOT/'material-batch-status.json').write_text(json.dumps({'asset':asset,'stage':'review_render_ready'})+'\n')
 except Exception:
