...
for name in ('front','back','left'):
    ...
    if name in ('front','back'):
        # Register torso width independently of a tail extending sideways in the card.
        band=(rows>y0+.15*(y1-y0))&(rows<y0+.65*(y1-y0))
        x0,x1=cols[band].min(),cols[band].max()
        body=(xyz[:,1]>.35)&(xyz[:,1]<.85)
        bx0,bx1=v[body,0].min(),v[body,0].max()
        u=(v[:,0]-bx0)/(bx1-bx0)
        if name=='back':u=1-u
    else:u=1-xyz[:,2]
    px=np.clip(np.rint(x0+u*(x1-x0)).astype(int),0,im.shape[1]-1)
    py=np.clip(np.rint(y1-xyz[:,1]*(y1-y0)).astype(int),0,im.shape[0]-1)
...
weights=np.stack((wfront,wback,wside),axis=1)
