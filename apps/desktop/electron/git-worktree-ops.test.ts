...
test('addWorktree: trims an explicit remote base before fetching and branching', async () => {
  const { cloneDir, remoteDir } = seedRemoteAndClone('trimmed-explicit-base', [])
  ...
  try {
    ...
    const result = await addWorktree(
      cloneDir,
      { base: '  origin/main\t', branch: 'trimmed-base', name: 'trimmed-base' },
      'git'
    )
