...
export function LunarCity({ onNewSession }: LunarCityProps) {
  ...
  const builtInAssets = [t.lunarCity.assetCompanions, t.lunarCity.assetScene, t.lunarCity.assetTextures]
  ...
  if (!enabled) {
    return (
      <section aria-label={t.lunarCity.disabled} className="flex h-full items-center justify-center p-8" role="status">
        <div className="max-w-md rounded-xl border border-(--ui-stroke-tertiary) bg-(--ui-bg-secondary) p-6 text-center">
          ...
          <h1 className="text-lg font-semibold">{t.lunarCity.disabled}</h1>
          <p className="mt-2 text-sm text-(--ui-text-tertiary)">{t.lunarCity.disabledHint}</p>
        </div>
      </section>
    )
  }
  ...
  return (
    <main aria-label={t.lunarCity.worldAriaLabel} className="h-full overflow-y-auto p-6">
      <div className="mx-auto flex max-w-4xl flex-col gap-6">
        <header>
          <div className="flex items-center gap-2 text-(--ui-text-tertiary)">
            <Globe className="size-4" />
            <span className="text-xs font-medium uppercase tracking-wide">{t.lunarCity.label}</span>
          </div>
          <h1 className="mt-2 text-2xl font-semibold">{t.lunarCity.heading}</h1>
          <p className="mt-1 max-w-2xl text-sm text-(--ui-text-tertiary)">{t.lunarCity.intro}</p>
        </header>
        ...
        <section className="rounded-2xl border border-(--ui-stroke-tertiary) bg-(--ui-bg-secondary) p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-medium">{t.lunarCity.defaultsActive}</h2>
              <p className="text-sm text-(--ui-text-tertiary)">{t.lunarCity.starterAssets}</p>
            </div>
          </div>
        ...
        {localAssets.length > 0 && (
          <section aria-label={t.lunarCity.localAssets} className="rounded-xl border border-(--ui-stroke-tertiary) p-4">
            <div className="flex items-center gap-2">
              <Box className="size-4" />
              <h2 className="font-medium">{t.lunarCity.localAssetsForReview}</h2>
            </div>
            ...
            <p className="mt-3 text-xs text-(--ui-text-tertiary)">{t.lunarCity.reviewOnly}</p>
          </section>
        )}
      </div>
      ...
      >
        <DialogContent aria-describedby="lunar-city-world-setup-description" aria-label={t.lunarCity.worldSetup}>
          <DialogHeader>
            <DialogTitle>{t.lunarCity.worldSetup}</DialogTitle>
            <DialogDescription id="lunar-city-world-setup-description">
              {t.lunarCity.worldSetupDescription}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-3">
            <div className="rounded-lg border border-(--ui-stroke-tertiary) bg-(--ui-bg-tertiary) p-3">
              <div className="flex items-center gap-2 font-medium">
                ...
                {t.lunarCity.builtInPets}
              </div>
              <img
                alt={t.lunarCity.builtInPetAlt}
                className="mt-3 size-16 rounded-lg object-cover"
                src={assetPath('hermes.png')}
              />
              <p className="mt-1 text-sm text-(--ui-text-tertiary)">{t.lunarCity.safeDefault}</p>
            </div>

            <div
              aria-label={t.lunarCity.dropLocalAssets}
            ...
            >
              ...
              <span className="text-sm font-medium">
                {dragging ? t.lunarCity.dropToStage : t.lunarCity.dropLocalAssets}
              </span>
              <span className="mt-1 text-xs text-(--ui-text-tertiary)">{t.lunarCity.formats}</span>
              <input
            ...
            {localAssets.length > 0 && (
              <p aria-live="polite" className="text-xs text-(--ui-text-tertiary)">
                {t.lunarCity.stagedFiles(localAssets.length)}
              </p>
            )}
          </div>
