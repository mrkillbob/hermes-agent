...
describe('Lunar City asset manifest', () => {
  ...
  it('requires masked reference silhouettes before future image-to-3D master generation', () => {
    ...
    for (const mask of maskManifest.masks) {
      ...
      // maskedSourceCachePath is generated by the local masking pipeline; only
      // enforce existence when that cache directory has been populated.
      if (existsSync('/private/tmp/lunar-city-master-asset-masked-sources')) {
